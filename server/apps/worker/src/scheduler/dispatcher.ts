 ...
 *   - 单员工并发 = 1（该员工无占用任务——running / waiting_approval（M3-06 口径）才启动）；
 *   - org 总并发 = 10（running 计数 < 10；waiting_approval 不占 org 并发，仅统计 running）。
...
import { EMPLOYEE_OCCUPYING_TASK_STATUSES } from '@tradepilot/shared';
...
export class Dispatcher {
  ...
  async tick(now = new Date()): Promise<number> {
    ...
    // ② 闸门计数（sched 上下文聚合）：员工占用 = running/waiting_approval（M3-06 挂起占员工位）；
    // org 并发仍仅统计 running（waiting_approval 不占 org 额度）
    const orgIds = [...new Set(queued.map((t) => t.orgId))];
    const employeeIds = [...new Set(queued.map((t) => t.employeeId))];
    const [orgRunning, empBusy] = await db.transaction(async (tx) => {
      await tx.execute(sql`SELECT set_config('app.sched', '1', true)`);
    ...
    for (const task of queued) {
      ...
      if (empUsed >= 1) {
        logger.debug(
          ...
          '员工并发=1 占用，保持 scheduled',
        );
        continue;
      }
      if (orgUsed >= ORG_CONCURRENCY_LIMIT) {
        logger.debug(
          { taskId: task.id, orgId: task.orgId, orgUsed },
          'org 并发已满，保持 scheduled',
        );
        continue;
      }
