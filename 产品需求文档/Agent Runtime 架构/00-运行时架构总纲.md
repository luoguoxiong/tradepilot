...
1. **挂起即冻结**：`waiting_approval` 不占并发额度（并发仅统计 `running`）；关联 `follow_up_task.status` 同步置 `waiting_approval`，`next_run_at` 保留原值冻结——Scheduler 扫描条件为 `status ∈ ('ready','scheduled') AND next_run_at <= now()`，挂起任务天然不被扫描。
