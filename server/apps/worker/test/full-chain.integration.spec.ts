...
const MAIL_ACCOUNT = `it-fullchain-${ORG.slice(-6)}@test.local`;
const CONTACT = createId('ct');
...
beforeAll(async () => {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(schema.contact).values({
      ...
      name: '全链路联系人',
      title: '采购经理',
      email: `it-fullchain-to-${ORG.slice(-6)}@test.local`,
    });
...
describe('全链路：lead_hunting 入队 → Runner → 图执行 → 落库 → SSE', () => {
  it('任务完成且全链路可观测', async () => {
    ...
    // 真实搜索命中数非确定性，仅做内部一致性断言
    const leadsOut = (task.outputs as Record<string, unknown>[])[0];
    expect(leadsOut?.['type']).toBe('leads');
    const payload = leadsOut?.['payload'] as { leads?: unknown[]; foundCount?: number };
    expect(Array.isArray(payload.leads)).toBe(true);
    expect(payload.foundCount).toBe(payload.leads?.length ?? 0);
    const [emp] = await db
    ...
    // 落库数量与 outputs 统计一致（真实搜索命中数不定）
    expect(leads.length).toBe(payload.leads?.length ?? 0);
...
describe('全链路：follow_up 审批挂起 → 批准 → resume 续跑 → 排期下一步', () => {
  it('medium 邮件默认人工审：挂起冻结，批准后续跑发送并排期第 2 步', async () => {
    ...
    // ② 批准（12 处置语义）→ resume 续跑：新鲜度校验通过 → 真实驱动外发 → 回写 → 排期第 2 步
    await db
