...
| 版本       | v0.4（2026-09-05，§4.7 新增「审批挂起与超时语义」（挂起即冻结排期 / next_run_at 单一写入口 / expired 终态级联 / 恢复新鲜度校验），§7 补审批超时行；P0-3 收口）<br>v0.3（2026-09-05，统一 email_send 风险级别 = medium（12 §7.1）：§4.3 示例拆分 send_email 节点、§4.5 riskLevel 枚举补 medium、§4.7 分级放行 + 强制人工例外、§5 时序图修正）<br>v0.2（2026-09-05，§10 五项全部澄清：并发=1/org 上限 10（14 §7.1）、时区基准=企业当地（14 §7.2）、LLM 配置 org 级+成本记录不熔断、日志保留默认 180 天、LLM 节点重试 2 次确认）<br>v0.1（2026-09-05） |
...
| 3   | **最小权限 + 受控闸门**    | 工具调用前强制校验 `ai_employee.permissions`；6 类受控动作必经 `approval_request`（12 §1.2；high 类永远人工审、medium 类可按规则自动放行），配置 `none` 返回 42201 |
...
| SOP         | `sop_template.content` (jsonb)             | 编译为 LangGraph 图（本文档 §4.3），**结构由本文档定义** |
...
- **并发控制**：同一 `ai_employee` 同时 active 任务（`running` 或审批挂起 `waiting_approval`，M3-06：挂起占员工位）数上限 **1**（员工卡片语义「当前任务」，LLM Agent 任务重资源、串行执行——挂起 ≠ idle，期间不得派发第二任务）；org 级总并发上限 MVP 内置常量 **10**（仅统计 `running`，挂起不占 org 额度）。超限行为 = **排队**：新任务置 `scheduled` 按时间 FIFO，当前任务终态后调度器自动启动队首（14 §7.1 已澄清）。
...
入队 ──▶ running ──┬─▶ completed（写 outputs）
                   ...
                   │         └────超时───▶ failed(approval_expired)，§4.7
...
```jsonc
{
  ...
  "nodes": [
    ...
    { "id": "crawl", "kind": "tool", "tool": "site_crawl", "logType": "crawl" },
    {
      "id": "score",
      "kind": "llm",
      "promptRef": "leadHunting.score",
      "outputSchema": "LeadScore",
      "logType": "match",
    },
    { "id": "save_leads", "kind": "tool", "tool": "crm_write", "risk": "low" },
    { "id": "draft_email", "kind": "llm", "promptRef": "sales.draft" },
    {
      "id": "send_email",
      "kind": "tool",
      "tool": "email_send",
...
- `risk: "medium" | "high"` 的节点执行前自动进入 Approval Gate（§4.7）：`high` 永远人工审；`medium` 按 16 FR-08 `autoApprove` 与员工 `approval_policy` 判定（email_send = medium，12 §7.1）。无需在图中手写 interrupt 逻辑。
...
interface ToolDefinition<I, O> {
  ...
  riskLevel: 'low' | 'medium' | 'high'; // medium/high → Approval Gate（放行策略见 §4.7）
  approvalType?:
    | 'email_send'
    | 'quote'
    | 'contract'
...
→ riskLevel 分流（low 直执行 / medium、high → Approval Gate，放行策略见 §4.7）
...
```text
medium/high 工具节点 → Gate（分级对齐 12 §7.1：high 永远人工审，medium 可按规则放行）
  ├─ 放行（medium 且 16 FR-08 已开 autoApprove + approval_policy.autoExecute 命中，
  │   且未命中强制人工例外）→ 直接执行，日志标记 auto
  └─ 需审批（所有 high；medium 未放行、或命中强制人工例外——如 Break-up Email（07 §4）、
      approval_policy "always"/"high_value_only"（02 §2））→ interrupt(payload)
              # 图挂起，state 已入 checkpointer
...
审批处置（12 §3.3）→ 回调 Worker → Command { resume }
  ├─ 批准     → 恢复新鲜度校验（见下 4）→ 图恢复，工具真实执行
  ├─ 编辑批准 → 以编辑后内容替换工具入参执行，差异留痕
  └─ 拒绝     → 拒绝原因写入 Org Memory（反馈闭环），图走拒绝分支收尾
```

**审批挂起与超时语义（v0.4，P0-3 收口）**：

1. **挂起即冻结（占用员工位，M3-06）**：`waiting_approval` 不占 **org 级** 并发额度（org 并发仅统计 `running`），但占**员工位**——挂起员工不再派发新任务，杜绝「resume 续跑撞上第二任务 → 双 running / 员工状态互相覆盖」；关联 `follow_up_task.status` 同步置 `waiting_approval`，`next_run_at` 保留原值冻结——Scheduler 扫描条件为 `status ∈ ('ready','scheduled') AND next_run_at <= now()`，挂起任务天然不被扫描。
2. **排期单一写入口**：`next_run_at` 仅允许两处写入——Scheduler 频控预检（不满足顺延）与图内 `schedule_next`（步进计算）；两者均不触达非 `scheduled` 状态任务，杜绝与审批挂起状态的覆盖竞争。
3. **超时终态**：`approval_request.expires_at` 到期（默认 48h，16 可配）→ `approval_request.status='expired'`（**终态，不可再处置**，`approval_log.action='expired'` 留痕）+ 邮件提醒经理一次（12 §7.2）；级联任务侧：`ai_task.status='failed'`（`error='approval_expired'`，已生成草稿随失败写 `outputs` 留存供手动取用）、`follow_up_task.status='paused'`（转人工：可在 07 手动发送 / 重新启用策略，或「重新生成」新任务走 `retry_of` 溯源）。
4. **恢复新鲜度校验**：批准 resume 后、发送前强制重跑新鲜度校验——`email_reply` 重查会话是否已被人工回复/关闭（已处理 → 不发送，转 `completed(insight)`）；`follow_up` 重跑 `check_replied`（已回复 → 走 `pause_strategy` 分支，07 §4），防止挂起窗口内客户已回复仍发机器邮件。

- **强制人工例外优先于自动放行**：Break-up Email 一律人工审（07 §4）；员工 `approval_policy.email_send = "always" / "high_value_only"` 时按其约束（02 §2）。命中任一例外时，即使 org 已开 autoApprove 也生成 `approval_request`（12 §7.1）。
- **红线**：6 类受控动作的 `approval_policy` 配置为 `none` 时任务创建即 42201（ER 图 §2.2 校验红线）。
- 审批超时提醒与升级（12 §7.2）由独立定时器扫描 `approval_request`，不触碰任务图执行；到期处置的级联动作（任务终态）见上方「审批挂起与超时语义」第 3 条。
...
- **可靠性取舍（M3-11）**：事件为「先写库、后 PUBLISH」，无事务 outbox——Worker 在终态事务提交与 PUBLISH done 之间崩溃会丢实时 done。权威源是 DB（`ai_task.status/finished_at/outputs`）：API SSE 开流后重读任务行（M3-03）+ 终态兜底轮询（非终态连接每 5s 重读，DB 已终态且未发过 done → 补发并关闭）覆盖丢失窗口；前端另有 `/tasks/{id}` 轮询兜底（04 §6.3）。
...
sequenceDiagram
    ...
    G->>AG: email_send（risk=medium）
    AG->>AU: interrupt → approval_request + waiting_approval
    AU-->>AG: 批准（回调）
    AG->>G: resume → 新鲜度校验 → 真实发送 → 写 log
    G-->>W: 完成
...
| 审批超时 | `approval_request.expires_at` 到期 → `status='expired'`（终态）+ 邮件提醒经理；级联 `ai_task.failed(approval_expired)` / `follow_up_task.paused` 转人工（§4.7 挂起与超时语义） |
