...
| 版本 | v0.4（2026-09-06，notification_setting.events jsonb 内容修正为「事件 × 渠道矩阵」口径（对齐接口 16 FR-09 与前端实现），channels 列改为聚合总开关，无 DDL 变更）<br>v0.3（2026-09-05，澄清多企业归属=维持一邮箱单企业（org_membership 列多租户专项）；org 三默认值列确认为 16 v0.4 区域与本地化口径，无 DDL 变更）<br>v0.2（2026-09-05，org 新增 `send_rules` 配置列（发送时间窗/频控），见 07 §7 与 16 FR-12）<br>v0.1（2026-09-04） |
...
| send_rules | jsonb | NOT NULL DEFAULT '{}' | 外发规则：`{ sendWindow: { start: "09:00", end: "18:00" }, minTouchIntervalDays: 3 }`（发送时间窗按 `timezone` 当地时刻；16 FR-12 / 07 §7） |
| created_at / updated_at | timestamptz | NOT NULL | 通用列 |

> **区域与本地化**（16 需求 v0.4 §3.3 FR-02）：`timezone / default_currency / default_language` 为 org 级默认值（07 发送窗口 / 09 报价默认币种 / 界面语言统一基准），v0.2 已建列，无 DDL 变更。
...
> **已澄清**（v0.3）：MVP 维持**一邮箱单企业**假设（`user_account.org_id` 单归属）；多企业归属（如咨询顾问）列多租户专项，届时再加 `org_membership` 关联表。
...
| id | text | PK | `ntf_` |
| org_id | text | NOT NULL FK→org | UNIQUE |
| channels | jsonb | NOT NULL | `{ site: true, email: false }` 聚合总开关（任一事件启用该渠道即为 true） |
| events | jsonb | NOT NULL | 事件 × 渠道矩阵：`{ approval_pending: { site: true, email: true }, risk_alert: { site: true, email: true }, task_failed: { site: true, email: false } }`（接口 16 FR-09 口径，v0.4.1） |
| created_at / updated_at | timestamptz | NOT NULL | |
...
CREATE TABLE org (
  ...
  send_rules       jsonb NOT NULL DEFAULT '{}',
