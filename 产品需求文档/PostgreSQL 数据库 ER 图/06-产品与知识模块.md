...
| embedding | vector(2048) | | pgvector 向量（维度按嵌入模型定，随 ai_model_setting 配置；P1 迁移 0005 由 1536 调整为 2048 以对齐所选模型） |
...
CREATE TABLE knowledge_chunk (
  ...
  token_count  integer,
  embedding    vector(2048), -- P1 迁移 0005：由 vector(1536) 调整（对齐所选嵌入模型维度）
  metadata     jsonb,
  created_at   timestamptz NOT NULL DEFAULT now(),
...
-- 注意：pgvector 对 vector 的 hnsw 索引上限为 2000 维；P1 列维度为 2048 → 需改用
-- CREATE INDEX ... USING hnsw (embedding halfvec_cosine_ops);（列类型改 halfvec(2048)）
```
