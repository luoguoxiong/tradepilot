...
| `isEmailSendConfigured()`     | 是否已注入（未注入 → `email_send` 走 mock 兜底）                |
...
- **归并一律按去重键**：公司/联系人归并键 = 归一化域名优先、名称兜底；禁止按公司名归并（mock 供应商
  公司名由查询词派生必然同名）。
- **mock 供应商确定性**：`MockSearchProvider` 额外产出的字段（员工数/国家）确定性可断言；真实供应商
  不产伪数据（无依据字段缺失）。
