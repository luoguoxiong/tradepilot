...
- **经营报告与一键边界（13 §7 已澄清）**：`business_report.period` 承载 `daily / weekly / monthly`（**本期仅按需手动生成**，定时自动列「后续增强 / Backlog」（13 v0.3 拍板）；Dashboard 每日报告复用 `daily` 手动接口）；报告统一五段结构（经营概览 / 机会洞察 / 风险预警 / 团队效率 / 建议与下一步）；`ai_discovery.suggestion.action` 白名单仅限发起类（`start_lead_task` / `enable_reactivation_strategy`），一键仅创建可撤销任务 / 策略，不产生对外或不可逆效果。
