/**
 * 01 Dashboard 工作台契约测试（01 接口文档 v0.2 · 接口规范 §2）：
...
import { api, contractServer, expectFail, expectOk } from './_server'
...
describe('GET /dashboard/summary 聚合契约（01 §3.1）', () => {
  ...
  it('D1：kpis 仅返回 new_customers/new_inquiries，每卡字段完整', async () => {
    const data = expectOk(
      (await api<DashboardSummary>('/dashboard/summary')).json,
    )
    const metrics = data.kpis.map((k) => k.metric)
    expect(metrics).toEqual(['new_customers', 'new_inquiries'])
    expect(metrics).not.toContain('new_quotes')
    expect(metrics).not.toContain('estimated_revenue')
  ...
  it('D2：pendingItems 仅 high_value_overdue/customer_reply，level/link 有效', async () => {
    ...
    expect(types).not.toContain('quote_approval')
    expect(types).not.toContain('order_delay_risk')
  ...
  it('FR-03：aiEmployees 字段完整，waiting_approval 携带 waitingApprovalCount', async () => {
    const data = expectOk(
      (await api<DashboardSummary>('/dashboard/summary')).json,
    )
    ...
    for (const employee of data.aiEmployees) {
      ...
      if (employee.status === 'waiting_approval') {
        expect(employee.waitingApprovalCount).toBeGreaterThan(0)
      }
    }
  })

  it('FR-04：highValueCustomers 字段完整且 score 为百分比', async () => {
    const data = expectOk(
      (await api<DashboardSummary>('/dashboard/summary')).json,
    )
    expect(data.highValueCustomers.length).toBeGreaterThan(0)
