...
| 版本 | v0.5（2026-09-05，`sales_order_item` 增 `cost_snapshot jsonb` 行级成本快照（10 v0.2.1，转订单复制 / 手工建单引擎核算），见 §2.4/§4）<br>v0.4（2026-09-05，`order_progress_log.updated_by` 说明收口（ERP/MES 列后续集成专项，10 v0.2 已澄清），无 DDL 变更）<br>v0.3（2026-09-05，订单进度/风险判定口径澄清（10 §7），见 §4，无 DDL 变更）<br>v0.2（2026-09-05，quotation_item 新增 `cost_snapshot jsonb` 行级成本快照（09 §7 成本/汇率口径澄清），见 §2.2/§4）<br>v0.1（2026-09-04） |
...
结构同 quotation_item：`id(oitem_) / org_id / sales_order_id FK / seq / product_id / product_name / quantity / unit_price / line_total / cost_snapshot jsonb NOT NULL DEFAULT '{}'（五项成本快照，转订单从报价行原样复制、手工建单由定价引擎按 09 §7 规则核算，成交后毛利率追溯，10 v0.2.1）`，`UNIQUE(sales_order_id, seq)`。
...
- **成本快照与汇率（09 §7 已澄清）**：五项成本（purchase/freight/insurance/tax/fx）由定价引擎按规则核算（purchase 取 `product.cost_price`，freight/insurance/tax 按 16 报价规则，fx 按报价时点汇率），`quotation_item.cost_snapshot` 行级固化（draft 可人工覆盖），报价级汇总展示；汇率 MVP 人工维护（`exchange_rate_source='manual'`，16 定期更新，第三方汇率 API 列**后续增强 / Backlog**），成交核算以报价时点快照为准。
...
- **转订单**：`POST /orders/from-quote` 校验 `quotation.status='won'`，复制明细进 `sales_order_item` 并记 `quotation_id` 溯源；订单行 `cost_snapshot` 从报价行**原样复制**（成本口径以报价时点快照为准，不按下单时点重算，10 v0.2.1）；手工建单由定价引擎按 09 §7 同一规则核算行级快照（成本缺失允许 `{}`）。
