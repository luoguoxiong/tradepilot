...
| decision_influence_pct | smallint | CHECK (0–100) | 决策影响力 |
...
- **创建任务**：`POST /lead-tasks` 先经 `POST /lead-tasks/parse`（AI 解析），最终写 `ai_task(input = { goalText, parsed, advancedSettings, targetCount })`；解析置信度随响应返回，不落库（03 §3.1）。
- **去重**：`add-to-crm` 按「同域名 / 同公司名」判定重复（口径待澄清，03 §3.4）；`website` + `lower(company_name)` 为比对键，命中时返回 `duplicated` 计数不建 customer。
