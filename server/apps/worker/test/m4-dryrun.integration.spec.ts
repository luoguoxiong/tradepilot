...
import { createSmtpImapDriver, type MailboxDriverRow } from '@tradepilot/integrations';
 ...
 * 真实 SmtpImapDriver ↔ GreenMail 测试邮箱（SMTP 1025 / IMAP 1114）+ 真实 LLM + 真实搜索供应商。
 * - lead_hunting：parse_goal→web_search（真实 Serper 兼容搜索）→match_product→find_contact
...
beforeAll(async () => {
  ...
  // ===== Runtime 装配（真实 LLM + 默认工具/流/提示词/输出契约注册表）=====
  ...
  // ===== M4 真实链路注入：email_send 真实外发出口（无 mock 兜底）=====
  // 搜索/嵌入供应商由 test/setup/providers.ts 装配为真实实现（Serper 兼容 + OpenAI embedding）
  ...
  processor = new EmailSyncProcessor({
    ...
    redis,
    logger,
    driverOptions: { encryptionKey: KEY, db },
  });
    ...
    await tx.insert(schema.customer).values({
      ...
      ownerId: (
        await tx
          ...
          .where(eq(schema.userAccount.orgId, ORG_ER))
          .limit(1)
      )[0]!.id,
    });
...
async function fetchInboxSubjects(
  account: string,
...
describe('M4 出口 dry-run · lead_hunting（真实搜索供应商）', () => {
  it('图执行 completed：web_search→评分→联系人→crm_write 落 ai_lead，LLM 记账全节点', async () => {
    ...
    // 真实搜索返回量取决于供应商结果（非确定性），只断言链路产出结构与分档字段落库完整
    ...
    expect(leads.every((l) => l.scoreLevel.length > 0)).toBe(true);
...
describe('M4 出口 dry-run · email_reply（真实 SMTP/IMAP 收发闭环）', () => {
  it('来信真实 IMAP 同步入库 → 派发 → 审批 → resume 真实 SMTP 外发 → 收件方 IMAP 收到', async () => {
    ...
    // ③ 图执行：grounded 由真实 LLM 依据知识库判定
    //    - true  → email_send（medium 无 autoApprove）→ 审批挂起 → 批准 resume → 真实 SMTP 外发
    ...
    // ⑤-⑦ 走发送分支时验证真实收发闭环；need_info 分支（grounded=false）不发送
    if (sent) {
