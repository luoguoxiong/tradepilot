 ...
 * - email_reply：语言检测 zh/en + 跟随（language 缺失按正文检测，外发 message.language 落 zh）、
 *   审批挂起→批准→resume→发送→writeback、类型化 outputs（draft+insight）；
 *   need_info 分支（grounded=false 不发送、missingInfo 留存）；
 ...
 * mock 契约：draftReply.grounded 经 z.literal 控制（happy=true / need_info=false），
 * subject='Mock 主题：合作意向确认'，intent.label='mock-label'。
...
beforeAll(async () => {
  ...
  // need_info 专用装配：draftReply.grounded = z.literal(false)（mock provider 对 literal 直出 value）
  const needInfoCompiler = new GraphCompiler({
    db,
    redis,
    logger,
    publisher,
  ...
  needInfoRunner = new TaskRunner({
    db,
    redis,
    logger,
    publisher,
    compiler: needInfoCompiler,
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(schema.org).values({ id: ORG_ER, name: 'M4-2 租户', timezone: 'Asia/Shanghai' });
    ...
    await tx.insert(schema.customer).values([
      ...
      { id: C2, orgId: ORG_ER, companyName: '缺料客户', country: 'US', ownerId: erAdmin },
    ]);
    await tx.insert(schema.conversation).values([
      { id: CONV1, orgId: ORG_ER, customerId: C1, channel: 'email', subject: '中文询价' },
      { id: CONV2, orgId: ORG_ER, customerId: C2, channel: 'email', subject: '缺料询价' },
    ]);
    ...
    await tx.insert(schema.message).values({
      ...
      senderType: 'contact',
      senderName: '客户',
      content: 'Hello, do you have titanium dioxide in stock?',
      status: 'sent',
      language: null,
      sentAt: new Date(Date.now() - 60_000),
      createdAt: new Date(Date.now() - 60_000),
    ...
    await tx.insert(schema.org).values({ id: ORG_FU, name: 'M4-3 租户', timezone: 'Asia/Shanghai' });
    ...
    await tx.insert(schema.followUpStrategy).values([
      { id: SA, orgId: ORG_FU, name: '两步策略', targetScope: {}, autoSendPolicy: 'auto_send', isDefault: false },
      { id: SB, orgId: ORG_FU, name: '三步含 Break-up', targetScope: {}, autoSendPolicy: 'auto_send', isDefault: false },
    ]);
    await tx.insert(schema.followUpStrategyStep).values([
      ...
      { id: SB_STEPS[0] as string, orgId: ORG_FU, strategyId: SB, seq: 1, dayOffset: 0, title: '首触' },
      { id: SB_STEPS[1] as string, orgId: ORG_FU, strategyId: SB, seq: 2, dayOffset: 3, title: '价值跟进' },
      { id: SB_STEPS[2] as string, orgId: ORG_FU, strategyId: SB, seq: 3, dayOffset: 7, title: 'Break-up', isBreakup: true },
    ]);
    await tx.insert(schema.customer).values([
    ...
    await tx.insert(schema.followUpTask).values([
      { id: FT1, orgId: ORG_FU, customerId: CF1, strategyId: SA, status: 'ready', nextRunAt: new Date() },
      { id: FT2, orgId: ORG_FU, customerId: CF2, strategyId: SB, status: 'ready', nextRunAt: new Date() },
      { id: FT3, orgId: ORG_FU, customerId: CF3, strategyId: SA, status: 'ready', nextRunAt: new Date() },
    ]);
...
describe('M4-2 email_reply：语言跟随 + 审批闭环 + 类型化 outputs', () => {
  it('中文来信（language 缺失按正文检测）：审批→批准→发送，外发 language=zh，writeback 落 CRM 活动', async () => {
    const taskId = await insertTask(ORG_ER, EMP_ER, 'email_reply', { inboxMessageId: MSG_ZH });

    // ① 首跑：draft grounded=true → email_send（medium，无 autoApprove）→ 挂起
    ...
    // ③ 外发消息：language=zh（检测兜底 + 跟随最近 in 信，06 §7）
    const [outMsg] = await db
      .select({ language: schema.message.language, direction: schema.message.direction, status: schema.message.status })
      .from(schema.message)
    ...
    // ⑤ 类型化 outputs（14 §1.2）：draft + insight（intent/copilot 即使发送也落）
    const outputs = resumed.outputs ?? [];
    expect(outputs.map((o) => o['type'])).toEqual(['draft', 'insight']);
    const draft = outputs[0]?.['payload'] as Record<string, unknown>;
    expect(draft['subject']).toBeTruthy();
    const insight = outputs[1]?.['payload'] as Record<string, unknown>;
  ...
  it('need_info：grounded=false 不发送，missingInfo 随 outputs 留存（completed · 需补充资料）', async () => {
    const taskId = await insertTask(ORG_ER, EMP_ER, 'email_reply', { inboxMessageId: MSG_NI });

    const result = await needInfoRunner.run(taskId);
    ...
    const outMsgs = await db
      ...
      .from(schema.message)
      .where(and(eq(schema.message.orgId, ORG_ER), eq(schema.message.direction, 'out')));
    expect(outMsgs).toHaveLength(1); // 仅上一用例的 1 条
    const approvals = await db
      .select({ id: schema.approvalRequest.id })
      .from(schema.approvalRequest)
      .where(eq(schema.approvalRequest.orgId, ORG_ER));
    expect(approvals).toHaveLength(1); // 仅上一用例的审批单

    // outputs：draft（grounded=false + missingInfo）+ insight（Copilot 仍落）
    const outputs = result.outputs ?? [];
    const draft = outputs.find((o) => o['type'] === 'draft')?.['payload'] as Record<string, unknown>;
    expect(draft['grounded']).toBe(false);
    expect((draft['missingInfo'] as unknown[]).length).toBeGreaterThan(0);
    expect(outputs.find((o) => o['type'] === 'insight')).toBeTruthy();

    // 未走 email_send 节点（步骤无「发送回复邮件」）
    const stepNames = (
      await db
...
describe('M4-3 follow_up：autoApprove 留痕 / Break-up 强制人工 / 策略走完', () => {
  it('autoApprove + autoExecute 命中：直发留痕（auto_approved 单 + approval_log），不挂起', async () => {
    ...
    const insight = result.outputs?.find((o) => o['type'] === 'insight')?.['payload'] as Record<string, unknown>;
